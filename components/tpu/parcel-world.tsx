...
export default function ParcelWorld({id,onTown,onTravel,suspended=false}:{id:number;onTown:()=>void;onTravel:()=>void;suspended?:boolean}){
 const auth=useAuth();const speech=useRef<SpeechStore>(new Map());useEffect(()=>{speech.current.clear()},[id]);const [chatTyping,setChatTyping]=useState(false),[socialError,setSocialError]=useState(''),[visitors,setVisitors]=useState(0);const peers=useRef<{username:string;x:number;y:number;title?:string;drawX:number;drawY:number}[]>([]);const [manage,setManage]=useState(false);
