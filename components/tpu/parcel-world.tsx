...
export default function ParcelWorld({id,onTown,onTravel,suspended=false}:{id:number;onTown:()=>void;onTravel:()=>void;suspended?:boolean}){
 ...
 const live=useRef({data,edit,selected,rotation,paused:false,stats,adminSkin:false,name:''});live.current={data,edit,selected,rotation,paused:suspended||shop||manage||busy||chatTyping,stats,adminSkin:auth.user?.role==='ADMIN',name:auth.user?.username??'Scavenger'};
 ...
 useEffect(()=>{let alive=true;Promise.all([1,4,7,10,99].map(n=>new Promise<void>((resolve,reject)=>{const image=new Image();image.onload=()=>{images.current[String(n)]=image;resolve()};image.onerror=()=>reject(Error('Could not load world graphics. Reload to retry.'));image.src=n===99?ADMIN_SKIN_URL:`/assets/sheet-${n}.webp`;}))).then(()=>{if(alive)setReady(true)}).catch(e=>{if(alive)setError(e.message)});return()=>{alive=false}},[]);
 ...
 useEffect(()=>{if(!data||!ready||!canvas.current||!viewport.current)return;
  ...
  function tick(now:number){const dt=Math.min((now-last)/1000,.04);last=now;const state=live.current;const d=state.data;if(!d){frame=requestAnimationFrame(tick);return}
   ...
   for(const peer of peers.current){const dx=peer.x-peer.drawX,dy=peer.y-peer.drawY;peer.drawX+=dx*(1-Math.exp(-dt*6));peer.drawY+=dy*(1-Math.exp(-dt*6));const walking=Math.hypot(dx,dy)>2;objects.push({y:peer.drawY,draw:()=>{g!.save();g!.translate(peer.drawX,peer.drawY);g!.fillStyle='#0007';g!.beginPath();g!.ellipse(0,1,23,9,0,0,Math.PI*2);g!.fill();g!.save();g!.scale(dx<0?-1:1,1);if(peer.adminSkin){drawAdminSkin(g!,images.current['99'],walking,now/6.5);}else if(walking){const f=Math.floor(now/130)%8,starts=[513,611,700,789,879,966,1055,1144],widths=[97,89,89,89,87,89,89,96];sprite(1,starts[f],204,widths[f],154,-widths[f]*.26,-78,widths[f]*.52,80)}else sprite(1,15,204,115,154,-30,-78,60,80);g!.restore();g!.textAlign='center';g!.font='12px monospace';g!.fillStyle='#78e2df';g!.fillText(peer.username,0,-90);if(peer.title){g!.fillStyle='#b9ed7c';g!.fillText(peer.title,0,-106);}g!.restore()}})}
   objects.push({y:p.y,draw:()=>{g!.fillStyle='#0007';g!.beginPath();g!.ellipse(p.x,p.y+1,23,9,0,0,Math.PI*2);g!.fill();g!.save();g!.translate(p.x,p.y);g!.scale(face,1);if(state.adminSkin){drawAdminSkin(g!,images.current['99'],moving,distance);}else if(moving){const f=Math.floor(distance/20)%8,starts=[513,611,700,789,879,966,1055,1144],widths=[97,89,89,89,87,89,89,96];sprite(1,starts[f],204,widths[f],154,-widths[f]*.26,-78,widths[f]*.52,80)}else sprite(1,15,204,115,154,-30,-78,60,80);g!.restore();g!.textAlign='center';g!.font='12px monospace';g!.fillStyle='#e6f2cd';g!.fillText(state.name,p.x,p.y-108);if(state.stats?.title){g!.fillStyle='#b9ed7c';g!.fillText(state.stats.title,p.x,p.y-124);}if(state.stats){g!.fillStyle='#081319';g!.fillRect(p.x-32,p.y-99,64,10);g!.fillStyle='#b9ee70';g!.fillRect(p.x-30,p.y-97,60*Math.max(0,Math.min(state.stats.maxHp,state.stats.hp))/state.stats.maxHp,6)}}});objects.sort((a,b)=>a.y-b.y).forEach(o=>o.draw());const speechNow=Date.now();drawSpeech(g!,speech.current,state.name,p.x,p.y,speechNow);for(const peer of peers.current)drawSpeech(g!,speech.current,peer.username,peer.drawX,peer.drawY,speechNow);
