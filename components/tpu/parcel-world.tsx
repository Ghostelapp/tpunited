...
import {drawSpeech,updateSpeech,followPlayer,type SpeechStore} from './world-presentation';
import {playerStats,legacyTitle,type GameState} from '@/packages/game-core/world';
...
export default function ParcelWorld({id,onTown,onTravel,suspended=false}:{id:number;onTown:()=>void;onTravel:()=>void;suspended?:boolean}){
 const auth=useAuth();const speech=useRef<SpeechStore>(new Map());useEffect(()=>{speech.current.clear()},[id]);const [chatTyping,setChatTyping]=useState(false),[socialError,setSocialError]=useState(''),[visitors,setVisitors]=useState(0);const peers=useRef<{username:string;x:number;y:number;adminSkin?:boolean;title?:string;drawX:number;drawY:number}[]>([]);const [manage,setManage]=useState(false);
 ...
 const [stats,setStats]=useState<{hp:number;maxHp:number;level?:number;title?:string}|null>(null);
 ...
 useHudLayout(viewport,false,ready&&!!data,String(id),0);
 const images=useRef<Record<string,HTMLImageElement>>({}),transform=useRef({x:0,y:0,zoom:1,w:0,h:0});
 const live=useRef({data,edit,selected,rotation,paused:false,stats,adminSkin:false,name:''});live.current={data,edit,selected,rotation,paused:suspended||shop||manage||busy||chatTyping,stats,adminSkin:['ADMIN','SUPER_ADMIN'].includes(auth.user?.role??''),name:auth.user?.username??'Scavenger'};
 ...
 useEffect(()=>{const controller=new AbortController();let alive=true;load().catch(e=>{if(alive)setError(e.message)});const timer=setInterval(()=>load().catch(e=>{if(alive){setError(e.message);setData(null)}}),30000);fetch('/api/game',{signal:controller.signal}).then(async r=>{if(!r.ok)return;const d=await r.json() as {character?:{state:GameState&{level?:number}}};if(alive&&d.character)setStats({hp:d.character.state.hp,maxHp:playerStats(d.character.state).hp,level:playerStats(d.character.state).level,title:legacyTitle(d.character.state)})}).catch(()=>{});return()=>{alive=false;controller.abort();clearInterval(timer)}},[load]);
 useEffect(()=>{let alive=true;Promise.all([1,4,7,10,99].map(n=>new Promise<void>((resolve,reject)=>{const image=new Image();image.onload=()=>{images.current[String(n)]=image;resolve()};image.onerror=()=>reject(Error('Could not load world graphics. Reload to retry.'));image.src=n===99?ADMIN_SKIN_URL:n===1?RACCOON_SKIN_URL:`/assets/sheet-${n}.webp?v=raccoon-20260914`;}))).then(()=>{if(alive)setReady(true)}).catch(e=>{if(alive)setError(e.message)});return()=>{alive=false}},[]);
 ...
 useEffect(()=>{if(!data||!ready||!canvas.current||!viewport.current)return;
  ...
  function tick(now:number){const dt=Math.min((now-last)/1000,.04);last=now;const state=live.current;const d=state.data;if(!d){frame=requestAnimationFrame(tick);return}
   ...
   const zoom=w<600?.65:w<850?.8:1;camera.current=followPlayer(camera.current,p,dt);transform.current={x:camera.current.x,y:camera.current.y,zoom,w,h};
   ...
   for(const peer of peers.current){const dx=peer.x-peer.drawX,dy=peer.y-peer.drawY;peer.drawX+=dx*(1-Math.exp(-dt*6));peer.drawY+=dy*(1-Math.exp(-dt*6));const walking=Math.hypot(dx,dy)>2;objects.push({y:peer.drawY,draw:()=>{g!.save();g!.translate(peer.drawX,peer.drawY);g!.fillStyle='#0007';g!.beginPath();g!.ellipse(0,1,23,9,0,0,Math.PI*2);g!.fill();g!.save();g!.scale(dx<0?-1:1,1);if(peer.adminSkin){drawAdminSkin(g!,images.current['99'],walking,now/6.5);}else drawRaccoonSkin(g!,images.current['1'],walking,now/6.5);g!.restore();g!.textAlign='center';g!.font='12px monospace';g!.fillStyle='#78e2df';g!.fillText(peer.username,0,-90);if(peer.title){g!.fillStyle='#b9ed7c';g!.fillText(peer.title,0,-106);}g!.restore()}})}
   objects.push({y:p.y,draw:()=>{g!.fillStyle='#0007';g!.beginPath();g!.ellipse(p.x,p.y+1,23,9,0,0,Math.PI*2);g!.fill();g!.save();g!.translate(p.x,p.y);g!.scale(face,1);if(state.adminSkin){drawAdminSkin(g!,images.current['99'],moving,distance);}else drawRaccoonSkin(g!,images.current['1'],moving,distance);g!.restore();g!.textAlign='center';g!.font='12px monospace';g!.fillStyle='#e6f2cd';g!.fillText(state.name,p.x,p.y-108);if(state.stats?.title){g!.fillStyle='#b9ed7c';g!.fillText(state.stats.title,p.x,p.y-124);}if(state.stats){g!.fillStyle='#081319';g!.fillRect(p.x-32,p.y-99,64,10);g!.fillStyle='#b9ee70';g!.fillRect(p.x-30,p.y-97,60*Math.max(0,Math.min(state.stats.maxHp,state.stats.hp))/state.stats.maxHp,6)}}});objects.sort((a,b)=>a.y-b.y).forEach(o=>o.draw());const speechNow=Date.now();drawSpeech(g!,speech.current,state.name,p.x,p.y,speechNow);for(const peer of peers.current)drawSpeech(g!,speech.current,peer.username,peer.drawX,peer.drawY,speechNow);
 ...
 {data&&ready&&<GameChat key={id} parcel={id} onTyping={setChatTyping} onMessages={messages=>updateSpeech(speech.current,messages)}/>}
 {(!data||!ready)&&<div className="parcel-load-overlay" role="status">{error||'Loading parcel…'}</div>}
 <div className="parcel-player-hud"><Frame/><Sprite width={42}/><div><strong>{auth.user?.username??'Scavenger'}</strong>{stats&&<><div className="parcel-hp"><i style={{width:`${100*Math.max(0,Math.min(stats.maxHp,stats.hp))/stats.maxHp}%`}}/></div><small>{stats.hp} / {stats.maxHp} HP</small></>}<small>{data?.canEdit?'YOUR LAND':'PRIVATE PARCEL'} · #{id}</small></div></div>
