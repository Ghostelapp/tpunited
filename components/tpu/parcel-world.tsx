...
export default function ParcelWorld({id,onTown,onTravel,suspended=false}:{id:number;onTown:()=>void;onTravel:()=>void;suspended?:boolean}){
 const auth=useAuth();const [manage,setManage]=useState(false);
 ...
 const live=useRef({data,edit,selected,rotation,paused:false,stats,name:''});live.current={data,edit,selected,rotation,paused:suspended||shop||manage||busy,stats,name:auth.user?.username??'Scavenger'};
 ...
 <div className="parcel-location-hud"><Frame/><strong>{data?.parcel.rarity??'LAND'}</strong><small>{data?.parcel.width} × {data?.parcel.height} tiles</small><small>WASD / arrows · tap to walk</small></div>
