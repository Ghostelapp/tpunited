...
export default function Litepaper(){return <div className={styles.paper}>
 ...
 <main id="main">
  <section className={styles.hero} aria-labelledby="paper-title">
   ...
   <div className={styles.heroPortrait}><LandingArtwork sheet={1} x={356} y={202} w={125} h={158} label="Raccoon Scavenger, the playable character"/></div>
  </section>
  <div className={styles.layout}>
   ...
   <div className={styles.chapters}>
    ...
    {paper.chapters.map(ch=><section id={ch.id} key={ch.id} className={styles.chapter} aria-labelledby={ch.id+'-title'}>
     ...
     {ch.visual==='world'&&<div className={styles.art}>{[{sheet:1,x:356,y:202,w:125,h:158,label:'RACCOON SCAVENGER'},{sheet:4,x:302,y:102,w:309,h:340,label:'SCRAP HOUSE'},{sheet:2,x:14,y:987,w:278,h:211,label:'GARBAGE KING'}].map(asset=><figure key={asset.label}><LandingArtwork {...asset}/><figcaption>{asset.label}</figcaption></figure>)}</div>}
     {ch.visual==='loop'&&<ol className={styles.loop} aria-label="Progression loop">{['Explore','Fight','Salvage','Upgrade','Return'].map((label,i)=><li key={label}><span>0{i+1}</span><div className={styles.loopArt}>{i===0?<LandingArtwork sheet={1} x={356} y={202} w={125} h={158} label="Raccoon Scavenger"/>:i===1?<LandingArtwork sheet={2} x={282} y={47} w={146} h={94} label="Trash Rat"/>:i===4?<LandingArtwork sheet={4} x={302} y={102} w={309} h={340} label="Scrap House"/>:<img src={'/assets/equipment/'+(i===2?'neon-blaster':'scavenger-hood')+'.svg'} alt={i===2?'Neon Blaster':'Scavenger Hood'} width={512} height={512}/>}</div>{label}</li>)}</ol>}
