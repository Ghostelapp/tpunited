...
import {LandingArtwork} from '@/components/tpu/landing-artwork';
...
export default function Litepaper(){return <div className={styles.paper}>
 ...
 <main id="main">
  <section className={styles.hero} aria-labelledby="paper-title">
   ...
   <div className={styles.heroPortrait}><LandingArtwork sheet={1} x={15} y={204} w={115} h={155} label="Panda Scavenger, the playable character"/></div>
  </section>
  <div className={styles.layout}>
   ...
   <div className={styles.chapters}>
    ...
    {paper.chapters.map(ch=><section id={ch.id} key={ch.id} className={styles.chapter} aria-labelledby={ch.id+'-title'}>
     ...
     {ch.visual==='world'&&<div className={styles.art}>{[{sheet:1,x:15,y:204,w:115,h:155,label:'PANDA SCAVENGER'},{sheet:4,x:302,y:102,w:309,h:340,label:'SCRAP HOUSE'},{sheet:2,x:14,y:987,w:278,h:211,label:'GARBAGE KING'}].map(asset=><figure key={asset.label}><LandingArtwork {...asset}/><figcaption>{asset.label}</figcaption></figure>)}</div>}
     {ch.visual==='loop'&&<ol className={styles.loop} aria-label="Progression loop">{['Explore','Fight','Salvage','Upgrade','Return'].map((label,i)=><li key={label}><span>0{i+1}</span><div className={styles.loopArt}>{i===0?<LandingArtwork sheet={1} x={15} y={204} w={115} h={155} label="Panda Scavenger"/>:i===1?<LandingArtwork sheet={2} x={282} y={47} w={146} h={94} label="Trash Rat"/>:i===4?<LandingArtwork sheet={4} x={302} y={102} w={309} h={340} label="Scrap House"/>:<img src={'/assets/equipment/'+(i===2?'neon-blaster':'scavenger-hood')+'.svg'} alt={i===2?'Neon Blaster':'Scavenger Hood'} width={512} height={512}/>}</div>{label}</li>)}</ol>}
     {ch.visual==='gear'&&<div className={styles.art}><figure><LandingArtwork sheet={4} x={302} y={102} w={309} h={340} label="Scrap House from the building atlas"/><figcaption>SCRAP HOUSE</figcaption></figure><figure><img src="/assets/equipment/neon-blaster.svg" alt="Neon Blaster from the current equipment artwork" width={512} height={512}/><figcaption>NEON BLASTER</figcaption></figure><figure><img src="/assets/equipment/scavenger-hood.svg" alt="Scavenger Hood from the current equipment artwork" width={512} height={512}/><figcaption>SCAVENGER HOOD</figcaption></figure></div>}
