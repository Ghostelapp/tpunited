...
To regenerate the PDF on Linux, install Python `reportlab`, `Pillow` and the DejaVu Sans fonts, then run `python scripts/build-litepaper.py` from the project root. The builder uses the original city image, selected sprite rectangles from game atlases 1, 2 and 4, and embedded PNG artwork from equipment SVG files. The web version uses the same rectangles through `LandingArtwork`. Render all pages and review layout after editing copy; fixed page layouts require manual spacing adjustments for longer text.
