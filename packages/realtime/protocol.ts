...
export type Peer={username:string;x:number;y:number;interior?:number;title?:string;hp:number;maxHp:number;lastAttack:number};
export type ChatMessage={id:string;username:string;message:string;created_at:number};
export type Snapshot={type:'snapshot';tick:number;time:number;ack:number;reset?:boolean;online:number;character:{username:string;state:GameState;revision:number};players:Peer[];hits:NonNullable<GameState['hits']>};
