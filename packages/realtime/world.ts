import {advance,applyMotion,initialState,migrateTown,type GameState,type Monster} from '../game-core/world.ts';
...
export function stepWorld(monsters:Monster[],players:WorldPlayer[],elapsed:number,now:number,rotation=0){
 const dt=Math.min(250,Math.max(0,elapsed));
 let shared=structuredClone(monsters);
 const moved=players.map(p=>{
  let state=migrateTown(p.state);
  const input=p.input?.scene===(state.interior??-1)?p.input:undefined;
